...
function DataComponent({ id }: { id?: string }) {
    ...
    const printQualifications = () => {
        return qualifications.map((qualification, indice)=>{
            return(
                <div key={indice} className={`${visible ? "fade-down flex flex-col md:flex-row justify-center items-center gap-3 md:gap-2 py-5 md:px-4 bg-gray-800/50  w-full rounded-4xl" : ""} hover:-translate-y-0.5 hover:outline hover:outline-gray-600/90 hover:shadow-gray-900/50 hover:shadow-lg transition-transform duration-100 select-none cursor-pointer`}
